...
class ChatGPTClient:
    ...
    def _click_send(self, page, before_count: int):
        ...
        # 2) 多策略尝试提交，每次后验证是否真的发出去了。三招轮着上：
        #    常规点击 → JS 直接派发 click（绕过 Grammarly/翻译插件浮层对指针事件的拦截，
        #    这是"发送按钮点击未生效"的常见根因）→ 回车。
        for _ in range(4):
            btn = page.locator(SEL["send"]).first
            try:
                if btn.is_visible() and btn.is_enabled():
        ...
        if src.startswith("http"):
            ...
            for attempt in range(2):
                try:
                    ...
                    pass
                time.sleep(0.8)
        # blob: 或同源图兜底：先试页内 fetch→FileReader；被 CSP 挡/失败时改用 canvas 导出
        #   （blob 与页面同源，画布不会被污染，toDataURL 可正常导出）。两条都失败才返回空串。
