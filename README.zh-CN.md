...
用**你自己的 ChatGPT 或 Gemini 账号额度**，自动完成「理解需求 → 组织提示词 → 生图 → 对比原图查篡改 → 迭代优化」的建筑渲染循环。你随时可以圈选局部修改、按点评继续，或提前输出。

**主要特性**：ChatGPT / Gemini（nano-banana）双生图引擎，都走你的订阅、免 API key（文本推理始终走 ChatGPT）；提示词助手支持**本地离线识图**（Ollama 视觉模型，免账号免 VPN）或 ChatGPT；成品可本地超分到 2K/4K/8K 并本地去水印（离线）；界面**中/EN 双语可切**（发给模型的提示词始终英文）；两个引擎都做了假死自愈——已出但没解码完的图不再被误判为卡死。
