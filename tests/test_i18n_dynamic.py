...
RUNTIME_STRINGS = [
    ...
    # —— Image#5/#6 状态栏泄漏：文本/生图重试、生图卡住尾注、下载失败、QC 下一步短语 ——
    "文本这一步没成功，自动干预后重试（第 2/3 次）…",   # kind=文本（旧规则误写"文字"）
    "生图这一步没成功，自动干预后重试（第 3/3 次）…",
    "⚠ 生图卡住：timeout —— 自动刷新重试一轮（剩余自动重试 2 次）…",  # 尾注也要翻
    "（下载生成图失败：OSError('boom')；已落 DOM 快照，本轮按未抓到图处理）",
    "第 3 轮检查完成（下一轮将从原图重画）：unacceptable, redraw needed",  # mode_label 短语
    "第 2 轮检查完成（下一轮将精修上一张）：looks good",
]
