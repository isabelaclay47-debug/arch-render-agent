...
# 建筑专用识图：动笔写提示词前，先像建筑师读图那样把原图的客观建筑事实看准、锁定。
# 目标是强化忠实度——让生成图严格还原这栋楼，而不是另画一栋"风格相似"的新楼。
ARCH_READ_STEP = """【第一步·先读原图（建筑专用识图）】动笔写提示词前，先像资深建筑师读图那样，把底图里**客观存在的建筑事实**逐项看准并锁定，别用"一栋现代建筑"这种泛泛描述敷衍：
- 建筑类型与用途气质（办公 / 文化 / 商业 / 居住 / 交通 / 教育等）；
- 体量与组合：几个体块、如何咬合或连接、主次与高低关系、各体块大致层数；
- 立面构成：开窗节奏与虚实比例、竖梃 / 横梁划分、柱网间距、基座—标准层—顶部收头的分段；
- 标志性构件：格栅、连廊、雨棚、坡道、幕墙分格、屋顶造型 / 女儿墙 / 构架 / 挑檐等——它们是这栋楼的"身份"，必须原样保留；
- 材质线索：各主要表面大致是什么材质（石材 / 清水混凝土 / 金属板 / 玻璃 / 木 / 砖…），认得出的就精确命名；
- 视角与环境：机位高度、透视类型（一点 / 两点）、周边关系、主光方向。
把这些观察**写进<理解>**让建筑师复核（若你认错了，他能当场纠正），并把其中的关键事实**明确写进英文提示词里"钉死"**，使生成图严格忠实于原图。"""
...
<英文提示词>
(The full English generation prompt, faithfully matching the Chinese one above. First restate the concrete architectural facts you identified in the base image — building type, massing, floor count, window rhythm, signature members (louvers/bridges/canopies/roof form), and materials — so the render stays faithful to THIS building. Then describe scene/style/materials/lighting/camera/atmosphere concretely. Do NOT repeat the universal quality/fidelity baseline — the system appends it automatically. English only.)
</英文提示词>"""
...
在读准原图之后，你的核心任务：把建筑师的想法**扩写成一份全面的生图提示词**——
- 建筑师明确提到的要求，逐条落实、写具体；
- 建筑师没提到的维度，由你凭专业经验替他做出最贴合其意图的决定并补全，不留空白；
- 有意向图时，先读懂它的光线/色调/材质气质，转译成文字写进提示词；
- 始终以第一步读到的原图建筑事实为准绳——提升的是材质/光影/配景/质感，绝不改动已识别的形体与构件。
...
- Keep genuine building signage that is physically part of the architecture legible and undistorted — never garbled.
- REMOVE and do NOT reproduce any watermark, logo stamp, semi-transparent text/graphic band, copyright notice, stock-photo or website overlay, brand mark, timestamp or caption that appears anywhere in the uploaded base or reference images. The render must be a clean on-site photograph with no overlaid text or translucent banding — especially across the sky or the middle of the frame.
- Photorealistic architectural photography — not cartoon, illustration, sketch, or concept art."""

# 画质档位 → 追加英文措辞。"标准"（默认档）也直接内置高画质用词，让默认出图就清晰
# 富细节——用户明确要求：默认档不改超分/模型，但把"高画质"措辞写进默认提示词。
QUALITY_HINTS = {
    "标准": "Render in high resolution with rich fine detail and sharp, well-resolved material "
            "textures; clean crisp edges, professional architectural photography quality; "
            "no softness, blur, smudging, noise or artifacts.",
...
# 参考图角色 → 发给生图 AI 的英文说明。generic=用户"不定义每张"时的通用参考；
# 其余=用户逐张指定的角色（材质/氛围意向/换入内容/图纸）。
REF_ROLE_EN = {
    "generic":  "a general REFERENCE — use it only for material, texture, color, lighting and atmosphere inspiration",
    "material": "a MATERIAL sample — apply its material look (color, texture, finish, reflectivity) to the matching surfaces, made to obey the base image's lighting and perspective",
...
def generation_message_multi(prompt_en: str, roles, quality: str = "标准",
                             ratio: str = "跟随原图") -> str:
    """底图 + 多张参考图（意向/材质/图纸）一起发给「生成对话」。
    roles：每张**参考图**（不含底图）一个角色键（见 REF_ROLE_EN），顺序与图片一致。
    全为 generic = 用户"不定义每张"；逐张指定 = 用户"定义材质/氛围/内容/图纸"。
    逐张枚举角色 + 强制"直接出图、不要描述/分析"——多图时模型很容易转去分析描述
    而非生成（用户实测 Gemini 收多张图会去分析）。底图只保几何构图。"""
    roles = list(roles or [])
    extras = " ".join(x for x in (QUALITY_HINTS.get(quality, ""),
                                  RATIO_HINTS.get(ratio, "")) if x)
    role_lines = "\n".join(
