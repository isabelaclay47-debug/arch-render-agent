...
(function () {
  ...
  const EXACT = {
    ...
    "— 可选多张；每张可选角色（通用/材质/氛围/换入内容/图纸/细部·必须保留），标「细部·必须保留」的会当强忠实约束、逐张进篡改检查；不选=通用参考，都会连同底图发给生图 AI": "— Optional, multiple allowed. Assign each a role (general / material / mood / content / drawing / detail-must-keep). Images marked \"Detail · must keep\" are treated as strict fidelity constraints and checked one by one; unassigned images are general references. All are sent to the image AI with the base image.",
    ...
    "从原图重画": "redraw from the base image",
    "精修上一张": "refine the previous image",
    "无结论": "no verdict",
    ...
    "检查通过": "Passed",
    "检查未完成": "Check incomplete",
    "需精修": "Needs refinement",
    "小瑕疵": "Minor flaws",
    "明显篡改": "Tampered",
    "已检查": "Checked",
    "严重": "Severe",
    ...
    /* 结果卡 / 状态日志里由 JS 动态渲染的中文（test_i18n 的静态 HTML 扫描覆盖不到，
       逐条补齐——对应用户"英文版要全方位"的诉求，尤其 Gemini 全包模式的运行日志）。 */
    "本轮后的提示词": "Prompt after this round",
    "本轮的修改指令": "Edit instruction this round",
    "（未解析出分析内容）": "(No analysis could be parsed)",
    "（未按标准格式返回，以下为导演原始回复）": "(Not in the standard format; the director's raw reply follows)",
    "已去除本轮生成图的 Gemini 水印。": "Removed the Gemini watermark from this round's image.",
    "已接管 Chrome，ChatGPT 登录状态正常。可以把专用 Chrome 最小化，回操作页面看进度。": "Chrome connected; ChatGPT is signed in. You can minimize the dedicated Chrome and watch progress on the app page.",
    "已接管 Chrome，Gemini 全包：一个页做导演文字推理、一个页专门生图，全程不启动 ChatGPT。": "Chrome connected. Gemini does everything: one tab for the director's text reasoning, one for image generation — ChatGPT is never launched.",
    "已接管 Chrome，Gemini（nano-banana）登录状态正常，用于生图。": "Chrome connected; Gemini (nano-banana) is signed in and ready to generate images.",
    "已重连 ChatGPT，继续任务。": "Reconnected to ChatGPT; resuming the task.",
    "已重连 Gemini，继续任务。": "Reconnected to Gemini; resuming the task.",
    "开新窗口多次失败，退回新标签方式。": "Opening a new window failed repeatedly; falling back to a new tab.",
    "换新标签仍未出图，改为**另开一个新窗口**重试…": "A new tab still produced no image; opening a brand-new window to retry…",
    "检测到浏览器/页面已断开，正在重连专用 Chrome 并重开 Gemini 页面…": "The browser/page was disconnected; reconnecting the dedicated Chrome and reopening the Gemini page…",
    "检测到浏览器/页面已断开，正在重连专用 Chrome 并重开页面…": "The browser/page was disconnected; reconnecting the dedicated Chrome and reopening the page…",
    "生图反复卡死——关闭所有 ChatGPT 页、重开干净页面重试（#5 关页重开兜底）…": "Image generation keeps stalling — closing all ChatGPT pages and reopening a clean page to retry…",
    "生图引擎：Gemini 全包——理解/提示词/查篡改/翻译与生图都由 Gemini 完成，全程不启动 ChatGPT。": "Image engine: Gemini does everything — understanding, prompting, fidelity checks, translation and image generation are all handled by Gemini; ChatGPT is never launched.",
    "生图引擎：Gemini（nano-banana）生图；ChatGPT 只做文本推理（理解/提示词/查篡改）。": "Image engine: Gemini (nano-banana) generates images; ChatGPT only does text reasoning (understanding / prompting / fidelity checks).",
    "请确认专用 Chrome 里 chatgpt.com 也已登录。下次「开始渲染」生效。": "Please make sure chatgpt.com is also signed in inside the dedicated Chrome. Applies to the next render.",
    "（未定位到生成图 <img>——落 DOM 快照供修选择器）": "(Could not locate the generated <img> — saved a DOM snapshot to help fix the selector.)",
    "⚠ 此图生成早于\"逐图提示词\"功能，无法精确到这一张；下面是该会话保留的提示词，供参考。": "⚠ This image predates the per-image prompt feature, so it can't be pinned to this exact image; below is the prompt kept for this session, for reference.",
    /* 阶段一"回复迟迟未出现/生成假死"等 reason 短语——供"…，自动刷新…页面重新检查…"整句拼接翻译 */
    "回复迟迟未出现，页面可能卡住": "The reply is taking too long; the page may be stuck",
    "生成似乎结束但图片没出现，页面可能假死": "Generation seems finished but no image appeared; the page may be unresponsive",
    "收到人工干预信号": "Manual-intervention signal received",
    "生成结束但未抓到生成图": "Generation ended but no generated image was captured",
    "切换失败": "Switch failed",
  ...
  const DYNAMIC = [
    ...
    // 生图卡住的尾注（上一条规则里 translateCore(m[2]) 会用到）：剩余自动重试次数。
    [/^（剩余自动重试 (\d+) 次）(…?)$/, m => ` (${m[1]} auto-retries left)${m[2]}`],
    // 抓图失败：整张下载异常，落 DOM 快照、本轮按未抓到图处理。
    [/^（下载生成图失败：(.+?)；已落 DOM 快照，本轮按未抓到图处理）$/, m => `(Failed to download the generated image: ${m[1]}; saved a DOM snapshot; treating this round as no image captured.)`],
    ...
    /* Gemini 全包运行日志（模型切换 / 重连 / 换页 / 抓图快照）——带占位符，逐条补 DYNAMIC */
    [/^Gemini 已是「(.+)」，无需切换。$/, m => `Gemini is already "${m[1]}"; no switch needed.`],
    [/^已在 Gemini 网页切到模型「(.+)」。$/, m => `Switched Gemini to the "${m[1]}" model on the web page.`],
    [/^Gemini 模型菜单里没找到「(.+)」——请在输入框右侧的模型按钮手动切换。$/, m => `"${m[1]}" was not found in Gemini's model menu — switch manually via the model button to the right of the input box.`],
    [/^没找到 Gemini 模型切换按钮——请在输入框右侧的模型按钮手动切到「(.+)」再继续。$/, m => `Could not find Gemini's model switch button — switch manually to "${m[1]}" via the model button to the right of the input box, then continue.`],
    [/^切换 Gemini 模型时出错（(.+?)）——请手动切到「(.+?)」，不影响其它功能。$/, m => `Error switching the Gemini model (${translateCore(m[1])}) — switch manually to "${m[2]}"; other features are unaffected.`],
    [/^连接调试端口失败（(.+?)），(\d+)\/(\d+) 退避重试…$/, m => `Failed to connect to the debug port (${translateCore(m[1])}); backing off and retrying ${m[2]}/${m[3]}…`],
    [/^重连失败（(.+?)），改为刷新\/换新对话再试一次。$/, m => `Reconnection failed (${translateCore(m[1])}); refreshing / opening a new chat to try once more.`],
    [/^重连失败（(.+?)），改为刷新页面再试一次。$/, m => `Reconnection failed (${translateCore(m[1])}); refreshing the page to try once more.`],
    [/^关页后重开失败（(.+?)），改为整体重连。$/, m => `Reopening after closing pages failed (${translateCore(m[1])}); falling back to a full reconnect.`],
    [/^开新窗口失败（(.+?)），重试（(\d+)\/(\d+)）…$/, m => `Opening a new window failed (${translateCore(m[1])}); retrying (${m[2]}/${m[3]})…`],
    [/^打开 Gemini 新对话失败（(.+?)），换个新标签重试（(\d+)\/(\d+)）…$/, m => `Failed to open a new Gemini chat (${translateCore(m[1])}); retrying in a new tab (${m[2]}/${m[3]})…`],
    [/^打开 ChatGPT 新对话失败（(.+?)），换个新标签重试（(\d+)\/(\d+)）…$/, m => `Failed to open a new ChatGPT chat (${translateCore(m[1])}); retrying in a new tab (${m[2]}/${m[3]})…`],
    [/^(文本|文字|生图)这一步没成功，自动干预后重试（第 (\d+)\/(\d+) 次）…$/, m => `The ${m[1] === "生图" ? "image-generation" : "text"} step did not succeed; auto-recovering and retrying (attempt ${m[2]}/${m[3]})…`],
    [/^(.+)，自动刷新 Gemini 页面重新检查…$/, m => `${translateCore(m[1])} — refreshing the Gemini page to re-check…`],
    [/^(.+)，自动刷新 ChatGPT 页面重新检查…$/, m => `${translateCore(m[1])} — refreshing the ChatGPT page to re-check…`],
    [/^（去水印跳过：(.+?)）$/, m => `(Watermark removal skipped: ${translateCore(m[1])})`],
    [/^⚠ 去水印不可用（缺依赖：(.+?)），已跳过。$/, m => `⚠ Watermark removal unavailable (missing dependency: ${m[1]}); skipped.`],
    [/^⚠ 去水印出错，已保留原图：(.+)$/, m => `⚠ Watermark removal errored; kept the original image: ${translateCore(m[1])}`],
    [/^（图已识别但取字节异常：scheme=(.+?) bytes=(.+?) src=(.+?)——已落 DOM 快照，发我可精修抓图）$/, m => `(Image detected but byte fetch failed: scheme=${m[1]} bytes=${m[2]} src=${m[3]} — saved a DOM snapshot; send it in to refine capture.)`],
    [/^（已存 Gemini 页面快照到 (.+?)，若确已出图却抓不到，把它发给开发者可精准修复）$/, m => `(Saved a Gemini page snapshot to ${m[1]}. If an image was produced but not captured, send it to the developer for a precise fix.)`],
    [/^（已存 ChatGPT 页面快照到 (.+?)，若确已出图却抓不到，发给开发者可精准修复选择器）$/, m => `(Saved a ChatGPT page snapshot to ${m[1]}. If an image was produced but not captured, send it to the developer to fix the selector.)`],
    [/^\[attach\] (\d+) 图上传：网络确认 (\d+)\/(\d+)，耗时 (.+?)s$/, m => `[attach] ${m[1]} image(s) uploaded: network-confirmed ${m[2]}/${m[3]}, took ${m[4]}s`]
  ];
